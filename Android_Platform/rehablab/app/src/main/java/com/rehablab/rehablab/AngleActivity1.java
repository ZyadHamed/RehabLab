...
public class AngleActivity1 extends AppCompatActivity {
    private TextView angleReading;
    private TextView statusIndicator;
    private TextView rangeInfo;
    
    // Define acceptable range
    private final double MIN_ACCEPTABLE_ANGLE_ARM_X = -5;
    private final double MAX_ACCEPTABLE_ANGLE_ARM_X = 95.5;
    private final double MIN_ACCEPTABLE_ANGLE_ARM_Y = -5;
    private final double MAX_ACCEPTABLE_ANGLE_ARM_Y = 5;
    private final double MIN_ACCEPTABLE_ANGLE_ARM_Z =-5;
    private final double MAX_ACCEPTABLE_ANGLE_ARM_Z = 5;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.angle_layout1);
        ...
        rangeInfo.setText("Acceptable range: " + MIN_ACCEPTABLE_ANGLE_ARM_X + "° : " + MAX_ACCEPTABLE_ANGLE_ARM_X + "°");
    ...
    private void updateAngleReading(float x , float y , float z) {
        ...
        if (y < MIN_ACCEPTABLE_ANGLE_ARM_Y ){
        ...
        }else if (y > MAX_ACCEPTABLE_ANGLE_ARM_Y) {
            statusIndicator.setText("Your hand is rotated to the right!!");
            statusIndicator.setBackgroundColor(Color.parseColor("#FFE0E0")); // Light red
            statusIndicator.setTextColor(Color.parseColor("#D32F2F")); // Dark red
        } else { if (z < MIN_ACCEPTABLE_ANGLE_ARM_Z ){
            statusIndicator.setText("Your hand is tilted to the left!!");
            statusIndicator.setBackgroundColor(Color.parseColor("#FFE0E0")); // Light red
            statusIndicator.setTextColor(Color.parseColor("#D32F2F")); // Dark red
        }else if (z > MAX_ACCEPTABLE_ANGLE_ARM_Z) {
            statusIndicator.setText("Your hand is tilted to the right!!");
            statusIndicator.setBackgroundColor(Color.parseColor("#FFE0E0")); // Light red
            statusIndicator.setTextColor(Color.parseColor("#D32F2F")); // Dark red
        }
        else {
            if (x >= MIN_ACCEPTABLE_ANGLE_ARM_X && x <= MAX_ACCEPTABLE_ANGLE_ARM_X) {
